...
public partial class ManufacturerTableViewModel : ReferenceTableViewModelBase
{
    ...
    private async Task DeleteAsync()
    {
        ...
        try
        {
            ...
            foreach (var item in itemsToDelete)
            {
                if (item.Id > 0)
                {
                    await _dataService.DeleteManufacturerCodeAsync(item.Id);
                }
                Items.Remove(item);
                deletedCount++;
            }
    ...
    private async Task ExportCsvAsync()
    {
        try
        {
            ...
            csv.AppendLine("Code,IndustryCode,Name");
            
            foreach (var item in Items)
            {
                csv.AppendLine($"\"{item.Code}\",\"{item.IndustryCode}\",\"{item.Name}\"");
            }
            ...
            for (int i = 1; i < lines.Length; i++)
            {
                ...
                if (values.Length >= 3)
                {
                    ...
                    { 
                        ...
                        IndustryCode = values[1].Trim('"').Trim(),
                        Name = values[2].Trim('"').Trim()
                    };
                    
                    // Check for duplicates
                    bool isDuplicate = Items.Any(existing => 
                        existing.Code.Equals(item.Code, StringComparison.OrdinalIgnoreCase) &&
                        existing.IndustryCode.Equals(item.IndustryCode, StringComparison.OrdinalIgnoreCase) &&
